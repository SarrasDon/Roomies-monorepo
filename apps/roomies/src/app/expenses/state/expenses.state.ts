...
import { createExpense, createExpenseFail, createExpenseSuccess, loadExpensesSuccess, setExpensesCount, setExpensesReasons } from './expenses.actions';
...
const _expenseReducer = createReducer<ExpensesState>(initialState,
  on(loadExpensesSuccess, (state, { expenses }) => expensesAdapter.upsertMany(expenses, state)),
  ...
  on(createExpenseSuccess, (state, { expense, clientId }) => {
    return expensesAdapter.updateOne({ id: clientId, changes: { ...expense } }, state); //TODO: test the id update
  }),
